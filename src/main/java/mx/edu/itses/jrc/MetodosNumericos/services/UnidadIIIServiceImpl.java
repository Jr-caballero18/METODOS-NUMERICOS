...
public Gauss AlgoritmoGauss(Gauss modelGauss) {
    if (modelGauss == null) return null;
    ...
    for (int i = 0; i < n; i++) b[i] = bList.get(i);
    ...
    out.setMatrizUEscalonada(flatten(A)); // U final
    out.setMatrizA(new ArrayList<>(Aflat)); // matriz original
    out.setVectorB(new ArrayList<>(bList)); // vector original
    out.setMensaje("OK");
    return out;
}
...
private static void swapRows(double[][] A, int r1, int r2) {
    double[] tmp = A[r1]; A[r1] = A[r2]; A[r2] = tmp;
}

    @Override
    public GaussJordan AlgoritmoGaussJordan(GaussJordan modelGJ) {
    if (modelGJ == null) return null;
    ...
    for (int i = 0, p = 0; i < n; i++)
        for (int j = 0; j < n; j++, p++)
            M[i][j] = Aflat.get(p);
    for (int i = 0; i < n; i++) M[i][n] = bList.get(i);
    ...
    for (int col = 0; col < n; col++) {
        ...
        if (piv != col) swapRowsAug(M, piv, col);
...
private static void swapRowsAug(double[][] M, int r1, int r2) {
    double[] tmp = M[r1]; M[r1] = M[r2]; M[r2] = tmp;
}
